  ...
  try {
    ...
    if (allowedListings === undefined || allowedListings === null) {
      return generateResponse(
        ...
        400,
        false,
        'allowedListings field is required',
        null
      );
    }
    ...
    if (typeof allowedListings !== 'number' || allowedListings < 0) {
      return generateResponse(
        ...
        400,
        false,
        'allowedListings must be a positive number',
        null
      );
    }

    const updatedUser = await updateUserAllowedListings(id, allowedListings);
    generateResponse(
      res,
      200,
      true,
      'User allowed listings updated successfully',
      updatedUser
    );
  } catch (error) {
  ...
  try {
    ...
    if (!req.files || Object.keys(req.files).length === 0) {
      return generateResponse(
        ...
        400,
        false,
        'At least one avatar image is required'
      );
    }
...
export const updateMultipleAvatarController = async (req, res) => {
  try {
    ...
    generateResponse(
      ...
      200,
      true,
      'Multiple avatars updated successfully',
      user
    );
  } catch (error) {
    console.error(error);
    generateResponse(
      ...
      500,
      false,
      'Failed to update multiple avatars',
      null
    );
  }
};
